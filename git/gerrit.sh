...
if [ ${changeid} = "0" ]; then
	...
	echo "Get commit-msg by \"scp -p -P 29418 10.38.38.210:hooks/commit-msg .git/hooks/\"."
...
project=$(echo $remote | awk '{print $2}' | awk -F/ '{print $NF}')
if [ -z ${project} ]; then
...
cmd="git push ssh://${user}@10.38.38.210:29418/${project}  HEAD:refs/for/${branch}"
